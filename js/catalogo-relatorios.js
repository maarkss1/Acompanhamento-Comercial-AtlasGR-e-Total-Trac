...
function criarResultadoCatalogo(chave,titulo,subtitulo,kpis,tabelas,nota="",extras={}){
  resultadoRelatorioCatalogo={chave,titulo,subtitulo,kpis,tabelas,nota,...extras};
...
// v41 — o modelo visual genérico (gerarHTMLRelatorioVisualGenerico e os
// helpers de gráfico/alertas) vive em js/relatorio-visual-generico.js.
  ...
  try{
    ...
    if(chave==="forecast_mensal"){
      ...
      criarResultadoCatalogo(chave,"Conversão Comercial • funil e Win Rate",`Coorte criada entre <strong>${escapeHtmlRelatorio(p.inicio||"início")}</strong> e <strong>${escapeHtmlRelatorio(p.fim||"hoje")}</strong>.`,
        ...
        [{titulo:"Funil de conversão — avanço por etapa",descricao:"A Taxa de avanço mostra quanto de quem chegou à etapa realmente seguiu adiante. Leia a Queda como vazamento: volume alto + avanço baixo indica o ponto que mais merece intervenção.",dados:rowsFunil,colunas:[{label:"Etapa",valor:"ETAPA"},{label:"Passaram",valor:"PASSARAM"},{label:"Avançaram p/ próxima",valor:"AVANCOU"},{label:"Taxa de avanço",valor:(x)=>x.TAXA_AVANCO_PCT===null?"—":`${x.TAXA_AVANCO_PCT}%`},{label:"Queda",valor:"QUEDA"}]},
         {titulo:"Conversão histórica por estágio",descricao:"Responde: quem passou por esta etapa, com que frequência terminou como cliente? Use para avaliar a qualidade histórica da passagem pela etapa, não como sequência do funil.",dados:rows,colunas:[{label:"Estágio",valor:"ESTAGIO"},{label:"Deals que passaram",valor:"VISITARAM"},{label:"Ganhos",valor:"GANHOS"},{label:"Conversão para ganho",valor:(x)=>`${x.CONVERSAO_PCT}%`}]},
         {titulo:"Comparativo — período anterior e mesma faixa no ano passado",descricao:"Compare o período atual com o anterior e com a mesma faixa do ano passado. Variações muito extremas pedem validação de cobertura histórica antes de virar conclusão de performance.",dados:rowsComparativo,colunas:[{label:"Métrica",valor:"METRICA"},{label:"Atual",valor:"ATUAL"},{label:"Período anterior",valor:"ANTERIOR"},{label:"Δ",valor:"DELTA"},{label:"Mesma faixa ano passado",valor:"ANO_ANTERIOR"},{label:"Δ YoY",valor:"DELTA_YOY"}]},
         {titulo:"Tendência do Win Rate — últimos 12 meses",descricao:"Leia os meses em ordem cronológica. Interprete o Win Rate junto com o número de decisões: meses com amostra pequena produzem taxas extremas sem representar tendência consolidada.",dados:rowsTendencia,colunas:[{label:"Mês",valor:"MES"},{label:"Oportunidades",valor:"OPORTUNIDADES"},{label:"Ganhos",valor:"GANHOS"},{label:"Perdas",valor:"PERDAS"},{label:"Win Rate",valor:(x)=>`${x.WIN_RATE_PCT}%`}]},
         {titulo:"Win Rate por vendedor",descricao:"Compare Win Rate com volume decidido. Uma taxa alta com 1 ou 2 decisões não deve valer mais que uma taxa consistente sobre dezenas de negócios.",dados:rowsVendedor,colunas:[{label:"Responsável",valor:"RESPONSAVEL"},{label:"Decididos",valor:"DECIDIDOS"},{label:"Ganhos",valor:"GANHOS"},{label:"Perdas",valor:"PERDAS"},{label:"Win Rate",valor:(x)=>`${x.WIN_RATE_PCT}%`},{label:"Receita",valor:(x)=>moedaRelatorio(x.RECEITA),html:true},{label:"Ticket médio",valor:(x)=>moedaRelatorio(x.TICKET),html:true},{label:"Em aberto",valor:"ABERTOS"}]},
         {titulo:"Win Rate por origem",descricao:"Uma origem saudável combina volume, Win Rate e receita. Evite escolher a \"melhor\" origem olhando somente a porcentagem de conversão.",dados:rowsOrigem,colunas:[{label:"Origem",valor:"ORIGEM"},{label:"Decididos",valor:"DECIDIDOS"},{label:"Ganhos",valor:"GANHOS"},{label:"Perdas",valor:"PERDAS"},{label:"Win Rate",valor:(x)=>`${x.WIN_RATE_PCT}%`},{label:"Receita",valor:(x)=>moedaRelatorio(x.RECEITA),html:true},{label:"Em aberto",valor:"ABERTOS"}]},
         {titulo:"Win Rate por produto",descricao:"Produtos devem ser avaliados por taxa e quantidade decidida. Taxas extremas com poucas decisões recebem menos peso gerencial.",dados:rowsProduto,colunas:[{label:"Produto",valor:"PRODUTO"},{label:"Decididos",valor:"DECIDIDOS"},{label:"Ganhos",valor:"GANHOS"},{label:"Perdas",valor:"PERDAS"},{label:"Win Rate",valor:(x)=>`${x.WIN_RATE_PCT}%`},{label:"Receita",valor:(x)=>moedaRelatorio(x.RECEITA),html:true}]},
         {titulo:"Motivos de perda",descricao:"Priorize os motivos que explicam a maior parte das perdas. \"Não especificado\" é também um problema de qualidade de CRM, porque impede aprender com a perda.",dados:rowsMotivo,colunas:[{label:"Motivo",valor:"MOTIVO"},{label:"Negócios",valor:"DEALS"},{label:"Valor perdido",valor:(x)=>moedaRelatorio(x.VALOR),html:true}]},
         {titulo:"Ciclo de vendas — distribuição (decididos)",descricao:"Compare mediana e média. Quando a média fica muito acima da mediana, poucos negócios lentos estão esticando o ciclo e merecem análise específica.",dados:rowsCiclo,colunas:[{label:"Faixa",valor:"FAIXA"},{label:"Negócios",valor:"NEGOCIOS"},{label:"% do total decidido",valor:(x)=>`${x.PARTICIPACAO_PCT}%`}]},
         {titulo:"Coorte mensal (mês de criação)",descricao:"Coortes recentes ainda estão \"verdes\": muitos negócios permanecem abertos. Compare Win Rate entre meses levando em conta quantos já tiveram decisão.",dados:rowsCoorte,colunas:[{label:"Mês",valor:"MES"},{label:"Oportunidades",valor:"OPORTUNIDADES"},{label:"Ganhos",valor:"GANHOS"},{label:"Perdas",valor:"PERDAS"},{label:"Em aberto/Piloto",valor:"ABERTOS"},{label:"Win Rate (fechados)",valor:(x)=>`${x.WIN_RATE_FECHADOS_PCT}%`}]}],
        ...
        {capitulos:[
          {antes:0,titulo:"Onde o volume perde tração.",descricao:"A leitura prioriza taxa de avanço, quedas absolutas e volume. Abra os dados apenas quando quiser validar etapa por etapa."},
          {antes:2,titulo:"Estamos melhorando ou apenas mudando a amostra?",descricao:"Compare período, tendência temporal e maturidade antes de interpretar saltos percentuais como ganho estrutural."},
          {antes:4,titulo:"Quem, de onde e com qual oferta.",descricao:"Vendedor, origem e produto devem ser lidos em conjunto com o volume decidido para evitar rankings enganosos por amostra pequena."},
          {antes:7,titulo:"Por que perdemos e quanto tempo levamos.",descricao:"Pareto de perdas e distribuição de ciclo mostram onde a operação pode recuperar eficiência sem depender de mais volume."}
        ]});
    }
