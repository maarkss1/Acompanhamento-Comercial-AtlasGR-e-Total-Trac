...
function cockpitHtmlKpiBlocos(cache, blocosIncluir) {
    ...
    linhas.forEach((l) => {
        var _a;
        if (blocosIncluir && !blocosIncluir.includes(l.bloco))
            return;
        (porBloco[_a = l.bloco] || (porBloco[_a] = [])).push(l);
    });
...
function cockpitHtmlAlertas(cache) {
    ...
    if (!info?.lista?.length)
        return `<p class="small-note">Sem alertas neste snapshot.</p>`;
    const icone = { critico: "🔴", atencao: "🟡", positivo: "🟢" };
    return `<div class="month-list">` + info.lista.map((a) => `<div class="vcard" style="padding:12px 16px;"><div class="vcard-name">${icone[a.nivel] || ""} ${escapeHtmlRelatorio(a.motivo)}</div>` +
        (a.valor ? `<div class="vcard-stats">${escapeHtmlRelatorio(a.valor)}</div>` : "") +
        `<div class="small-note">Ação sugerida: ${escapeHtmlRelatorio(a.acao)}</div></div>`).join("") + `</div>`;
}
