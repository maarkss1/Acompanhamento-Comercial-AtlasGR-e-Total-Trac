...
// js/cockpit.js pra ts/cockpit.ts, sem nenhuma tipagem ainda — o objetivo
// deste passo é só provar que o pipeline de build (tsc → js/cockpit.js,
// verificado em CI) funciona de ponta a ponta, sem risco de mudar
// comportamento. `@ts-nocheck` desliga a checagem de tipos por enquanto —
// remover essa linha e tipar de verdade (a começar por CockpitState, Deal e
// pelas funções de conciliação Comercial↔Financeiro, exatamente onde vivia
// o bug do Win Rate) é o PRÓXIMO passo, feito arquivo por arquivo, nunca
// tudo de uma vez. NÃO editar js/cockpit.js diretamente a partir de agora —
// ele é gerado por `npm run build` a partir deste arquivo; editar o .js
// direto se perde no próximo build e diverge do que o CI valida.
// ---------------------------------------------------------------------------
...
function alternarCockpitCompleto() {
    ...
    if (btn)
        btn.textContent = recolhido ? "▾ Ver painel completo" : "▴ Recolher painel";
}
function expandirCockpitCompleto() {
    const card = cockpitEl("cockpit-executivo");
    if (card && card.classList.contains("cockpit-recolhido"))
        alternarCockpitCompleto();
    if (typeof revelarFluxoExtracao === "function")
...
function iniciarCockpitExecutivo() {
    ...
    const hoje = new Date();
    if (simMes)
        simMes.value = String(hoje.getMonth() + 1);
    if (simAno && !simAno.value)
        simAno.value = String(hoje.getFullYear());
...
function cockpitTickerItens() {
    ...
    if (!cache)
        return ["Sem dados carregados nesta sessão — abra um relatório abaixo ou configure o webhook para atualizar automaticamente."];
    ...
    if (g)
        itens.push(`Ritmo de geração de pipeline: ${cockpitND(g.paceRitmoPct, (v) => `${v}%`)}`);
...
function cockpitAtualizarTicker() {
    ...
    if (label)
        label.title = cockpitState.ultimaAtualizacao ? `Atualizado às ${cockpitState.ultimaAtualizacao.toLocaleTimeString("pt-BR")}` : "Ainda sem dados nesta sessão";
}
...
function cockpitIniciarAutoAtualizacao() {
    if (cockpitAutoAtualizacaoTimer)
        return;
    cockpitAutoAtualizacaoTimer = setInterval(() => {
        if (cockpitState.carregando)
            return;
...
function atualizarRelogioCockpit() {
    ...
    if (!cockpitState.ultimaAtualizacao) {
        ...
        if (elHome)
            elHome.textContent = "Última atualização: ainda não carregado nesta sessão.";
        return;
    }
...
function cockpitSimularMesAno() {
    ...
    if (!ano || (!anoInteiro && !mes)) {
        alert("Escolha mês e ano pra simular (ou marque \"Ano inteiro\").");
        return;
    }
    ...
    if (statusEl)
        statusEl.textContent = `🔎 Simulando ${rotuloPeriodo} — todo o Cockpit (Resultado, Forecast, Win Rate, Proteção de Receita) passa a tratar esse período como "atual", até você clicar em "🗑️ Limpar".`;
...
function cockpitLimparSimulacao() {
    ...
    const hoje = new Date();
    if (cockpitEl("cockpitSimMes"))
        cockpitEl("cockpitSimMes").value = String(hoje.getMonth() + 1);
    if (cockpitEl("cockpitSimAno"))
        cockpitEl("cockpitSimAno").value = String(hoje.getFullYear());
    const chkAnoInteiro = cockpitEl("cockpitSimAnoInteiro");
    if (chkAnoInteiro)
        chkAnoInteiro.checked = false;
}
async function carregarVendedoresCockpit() {
    ...
    try {
        ...
        if ([...sel.options].some((o) => o.value === anterior))
            sel.value = anterior;
    }
    catch (e) {
...
async function cockpitCarregarReunioes() {
    ...
    if (status)
        status.textContent = "Carregando reuniões do Bitrix (pode levar alguns segundos)...";
    try {
...
function cockpitRenderReunioes() {
    ...
    if (!base) {
        ...
        if (kpisEl)
            kpisEl.innerHTML = cockpitPlaceholderVazio('Clique em "↻ Carregar reuniões" para buscar as reuniões (atividade + etapa do funil de Leads) do período selecionado no filtro acima.');
        tabelas.forEach((id) => { const el = cockpitEl(id); if (el)
            el.innerHTML = ""; });
        return;
    }
    ...
    const linhas = r.linhas.filter((x) => {
        if (usuario && x._RESPONSAVEL_ID !== String(usuario))
            return false;
        if (pipeline && x._PIPELINE_ID !== String(pipeline))
            return false;
        return true;
    });
    ...
    if (status)
        status.textContent = `${r.linhas.length} reunião(ões) no período (atividade + etapa do Lead) · ${linhas.length} após filtro de vendedor/pipeline · atualizado às ${new Date().toLocaleTimeString("pt-BR")}.`;
}
...
async function aplicarFiltroProdutoCockpit() {
    ...
    if (!termo) {
        ...
        cockpitState.dealsFinanceiroFiltrados = cockpitFinanceiroConciliadoComFiltro(cockpitState.dealsFinanceiro || [], cockpitState.dealsFiltrados, !!(vendedor || origem));
        if (status)
            status.textContent = "";
        renderizarCockpit();
        return;
    ...
    if (status)
        status.textContent = "Buscando produtos dos negócios carregados (pode levar alguns segundos)...";
    ...
    cockpitState.dealsFinanceiroFiltrados = cockpitFinanceiroConciliadoComFiltro(cockpitState.dealsFinanceiro || [], comProduto, true);
    if (status)
...
function cockpitFiltrarPorVendedorOrigem(deals) {
    ...
    return deals.filter((d) => {
        if (vendedor && idBitrixString(d.ASSIGNED_BY_ID) !== String(vendedor))
            return false;
        if (origem && String(d.SOURCE_ID || "") !== String(origem))
            return false;
        return true;
    });
}
...
async function atualizarCockpit() {
    ...
    if ((cockpitEl("cockpitVendedor")?.options.length || 0) <= 1)
        carregarVendedoresCockpit();
    if ((cockpitEl("cockpitOrigem")?.options.length || 0) <= 1)
        carregarOrigensCockpit();
    try {
        ...
        cockpitState.dealsFinanceiroFiltrados = cockpitFinanceiroConciliadoComFiltro(dealsFinanceiro, cockpitState.dealsFiltrados, !!(vendedorAtual || origemAtual));
...
function cockpitVerificarElegibilidade(d, refISO) {
    ...
    if (!(Number(d._VALOR) > 0))
        motivos.push("Sem valor válido");
    if (!parteDataISO(d.CLOSEDATE))
        motivos.push("Sem data prevista de fechamento (CLOSEDATE)");
    if (!idBitrixValido(d.ASSIGNED_BY_ID))
...
function cockpitCoverageRecomendado(winRate) {
    if (winRate == null || !Number.isFinite(winRate) || winRate <= 0)
        return null;
...
function cockpitSituacaoFinanceiroPorCliente(dealsFinanceiro) {
    const mapa = {};
    (dealsFinanceiro || []).forEach((d) => { var _a; (mapa[_a = chaveClienteDealModelo(d)] || (mapa[_a] = [])).push(d); });
    return mapa;
}
...
function cockpitClassificarComercialFinanceiro(deals, dealsFinanceiro, filtro) {
    ...
    return candidatos.map((d) => {
        if (d._SEMANTICA === "failure")
            return { ...d, _RESULTADO: "perda" };
        const fin = mapaFin[chaveClienteDealModelo(d)];
        if (fin && fin.some((f) => cockpitEhFechadoFinanceiro(f)))
            return { ...d, _RESULTADO: "ganho" };
        if (fin && fin.some((f) => f._SEMANTICA === "failure" || normalizarTextoChave(f._ESTAGIO || "").includes("cancelado")))
            return { ...d, _RESULTADO: "perda" };
...
function cockpitCalcular() {
    ...
    let coverage = null;
    if (gapMeta === 0)
        coverage = "meta batida";
    else if (gapMeta != null && gapMeta > 0)
        coverage = pipelineElegivel / gapMeta;
...
function cockpitStatusProtecao(coverage) {
    ...
    if (coverage < 2)
        return { rotulo: "crítico", classe: "cockpit-status-critico" };
    if (coverage < 3)
        return { rotulo: "atenção", classe: "cockpit-status-atencao" };
...
function cockpitCalcularAlertas(c, g) {
    ...
    if (ticketBase > 0) {
        ...
        const grandesInativos = (c.deals || []).filter(d => {
            if (d._SEMANTICA !== "process" || d._VALOR < limiarGrande)
                return false;
...
function cockpitGerarSituacaoAgora() {
    const cache = cockpitState.ultimoCalculo;
    if (!cache) {
        mostrarErro('Clique em "↻ Atualizar agora" no Cockpit antes de gerar a Situação Comercial Agora.');
        return;
    }
    const { c, g, alertasInfo } = cache;
...
function fecharSituacaoCockpitPorFundo(ev) {
    if (ev.target && ev.target.id === "cockpitSituacaoModal")
        fecharSituacaoCockpit();
}
// ---------------------------------------------------------------------------
// Renderização
// ---------------------------------------------------------------------------
...
function cockpitGerarResumoIA() {
    ...
    if (!c) {
        alert("Sem dados carregados para análise. Atualize o Cockpit primeiro.");
        return;
    }
    if (typeof iaAbrirModalAprofundamento === "function") {
        iaAbrirModalAprofundamento({ modo: "Cockpit Comercial Executivo", conteudo: c }, "Cockpit Comercial · Resumo Executivo & Diagnóstico com IA");
    }
    else {
...
function cockpitAnimarValor(el) {
    ...
    if (!partes)
        return; // "não disponível", "—" etc. — deixa como está
    if (el.dataset.cockpitAnimando === "1")
        return;
    const anteriorTexto = el.dataset.cockpitUltimoTexto;
    if (anteriorTexto === textoFinal)
        return; // já é o valor mostrado, nada a animar
    ...
    if (card)
        card.classList.add("cockpit-kpi-oscilando");
    ...
    function passo(agora) {
        ...
        if (p < 1) {
            ...
            if (card)
                card.classList.remove("cockpit-kpi-oscilando");
        }
    }
    requestAnimationFrame(passo);
}
...
function cockpitIniciarAnimacaoValores() {
    if (cockpitObservadorValoresIniciado)
        return;
...
function cockpitRenderEstadoVazio() {
    ...
    COCKPIT_CONTAINERS_KPI.forEach((id) => { const el = cockpitEl(id); if (el)
        el.innerHTML = cockpitPlaceholderVazio(msg); });
    COCKPIT_CONTAINERS_LISTA.forEach((id) => { const el = cockpitEl(id); if (el)
        el.innerHTML = cockpitPlaceholderVazio(msg); });
    const protecao = cockpitEl("cockpitProtecaoTabela");
    if (protecao)
        protecao.innerHTML = cockpitPlaceholderVazio(msg);
...
function renderizarCockpit() {
    ...
    if (cockpitEl("cockpitResultadoMes"))
        cockpitEl("cockpitResultadoMes").innerHTML = resultadoMesHtml;
    if (cockpitEl("homeResultadoMes"))
        cockpitEl("homeResultadoMes").innerHTML = resultadoMesHtml;
    ...
    if (cockpitEl("cockpitForecast"))
        cockpitEl("cockpitForecast").innerHTML = forecastHtml;
    if (cockpitEl("homeForecast"))
        cockpitEl("homeForecast").innerHTML = forecastHtml;
    // C) Saúde do Pipeline
    ...
    if (cockpitEl("cockpitSaudePipeline"))
        cockpitEl("cockpitSaudePipeline").innerHTML = saudePipelineHtml;
    if (cockpitEl("homeSaudePipeline"))
        cockpitEl("homeSaudePipeline").innerHTML = saudePipelineHtml;
    ...
    if (cockpitEl("cockpitWinRate"))
        cockpitEl("cockpitWinRate").innerHTML = winRateHtml;
    if (cockpitEl("homeWinRate")) {
        cockpitEl("homeWinRate").innerHTML = winRateHtml;
        if (cockpitEl("homeWinRateSection"))
            cockpitEl("homeWinRateSection").style.display = "block";
    }
    ...
    if (cockpitEl("cockpitEficiencia"))
        cockpitEl("cockpitEficiencia").innerHTML = eficienciaHtml;
    if (cockpitEl("homeEficiencia"))
        cockpitEl("homeEficiencia").innerHTML = eficienciaHtml;
...
function cockpitAbrirDrill(chave, titulo) {
    ...
    if (COCKPIT_DRILL_REUNIOES.has(chave)) {
        ...
        if (chave === "contratosNaoAssinados" || chave === "contratosNaoAssinadosAtrasados")
            colunas.push({ label: "Dias parado na etapa", valor: (x) => x._DIAS_PARADO === "" ? "—" : `${x._DIAS_PARADO}d` });
    }
...
function fecharDrillCockpitPorFundo(ev) {
    if (ev.target && ev.target.id === "cockpitDrillModal")
        fecharDrillCockpit();
}
// ---------------------------------------------------------------------------
...
function cockpitExigirCache() {
    const cache = cockpitState.ultimoCalculo;
    if (!cache) {
        mostrarErro('Clique em "↻ Atualizar agora" no Cockpit antes de exportar (nada foi calculado ainda nesta sessão).');
        return null;
    }
    return cache;
}
...
function cockpitFormatarValorTexto(valor, unidade) {
    if (valor == null || valor === "não disponível")
        return "não disponível";
    const n = Number(valor);
    if (!Number.isFinite(n))
        return String(valor);
    if (unidade === "R$")
        return moedaRelatorio(n);
    if (unidade === "%")
        return `${valor}%`;
    if (unidade === "dias")
        return `${valor} dia${n === 1 ? "" : "s"}`;
    if (unidade === "x")
        return `${valor}x`;
    if (unidade === "pp")
        return `${valor} p.p.`;
...
function cockpitMontarResultadoVisual(completo) {
    ...
    const linhas = cockpitListaKpisExport(cache);
    const porBloco = {};
    linhas.forEach((l) => { var _a; (porBloco[_a = l.bloco] || (porBloco[_a] = [])).push(l); });
    ...
    ["Forecast total do mês", "Gap do Forecast"].forEach((nome) => {
        ...
        if (it)
            kpis.push(kpi(it.indicador, cockpitFormatarValorTexto(it.valor, it.unidade)));
    });
    const tabelaIndicadorValor = (titulo, itens, excluir = []) => {
    ...
    if (g)
        tabelas.push(tabelaIndicadorValor("Geração de Pipeline", porBloco["Geração de Pipeline"]));
    if (s)
        tabelas.push(tabelaIndicadorValor("SDR (resumo)", porBloco["SDR (resumo)"]));
    if (q) {
        tabelas.push({
    ...
    const capitulos = [];
    if (idx("Forecast") > -1)
        capitulos.push({ antes: idx("Forecast"), titulo: "Previsão e cobertura de meta.", descricao: "Forecast, saúde do pipeline e proteção de receita mês a mês." });
    if (idx("Alertas Gerenciais") > -1)
        capitulos.push({ antes: idx("Alertas Gerenciais"), titulo: "O que pede decisão agora.", descricao: "Alertas gerados automaticamente a partir dos mesmos dados do snapshot." });
    if (idx("Eficiência da Máquina") > -1)
        capitulos.push({ antes: idx("Eficiência da Máquina"), titulo: "Como a máquina comercial está performando.", descricao: "Win Rate, ciclo de vendas e confirmação Comercial → Financeiro." });
...
function cockpitAbrirHTMLExport() {
    ...
    if (h)
        mostrarRelatorioVisualInline(h, "Cockpit Comercial");
}
function cockpitBaixarHTMLExport() {
    const h = cockpitGerarHTMLExport(false);
    if (h)
        baixarArquivo(h, `cockpit_comercial_${dataHoje()}.html`, "text/html;charset=utf-8;");
}
function cockpitAbrirRelatorioExecutivo() {
    const h = cockpitGerarHTMLExport(true);
    if (h)
        mostrarRelatorioVisualInline(h, "Relatório Executivo Completo — Cockpit");
}
...
function cockpitRenderizarGrafico(c) {
    ...
    if (graficoCockpitInstance)
        graficoCockpitInstance.destroy();
...
function cockpitRenderizarMetasDesdobradas(c) {
    ...
    try {
        salvo = JSON.parse(localStorage.getItem("atlas-metas-desdobradas" + (typeof marcaAtiva === "function" ? marcaAtiva().sufixoStorage : ""))) || {};
    }
    catch (e) { }
    let html = "";
...
window.cockpitSalvarMetasIndividuais = function () {
    ...
    try {
        localStorage.setItem("atlas-metas-desdobradas" + (typeof marcaAtiva === "function" ? marcaAtiva().sufixoStorage : ""), JSON.stringify(metas));
    }
    catch (e) { }
...
function salvarOrdemLayout() {
    ...
    // vazava/sobrescrevia a da outra no mesmo navegador.
    try {
        localStorage.setItem('atlas-layout-ordem' + (typeof marcaAtiva === "function" ? marcaAtiva().sufixoStorage : ""), JSON.stringify(ids));
    }
    catch (e) { }
}
...
function cockpitTelaoMostrarSlide(indice) {
    COCKPIT_TELAO_BLOCOS.forEach((id, i) => {
        const el = cockpitEl(id);
        if (el)
            el.classList.toggle("cockpit-telao-slide-ativo", i === indice);
    });
    const indicador = cockpitEl("cockpitTelaoIndicador");
    if (indicador)
        indicador.textContent = `${indice + 1} / ${COCKPIT_TELAO_BLOCOS.length}`;
...
function cockpitAtivarModoTelao() {
    if (document.body.classList.contains("cockpit-modo-telao"))
        return;
...
function cockpitDesativarModoTelao() {
    ...
    if (cockpitTelaoTimerSlide) {
        clearInterval(cockpitTelaoTimerSlide);
        cockpitTelaoTimerSlide = null;
    }
    if (cockpitTelaoTimerRelogio) {
        clearInterval(cockpitTelaoTimerRelogio);
        cockpitTelaoTimerRelogio = null;
    }
    COCKPIT_TELAO_BLOCOS.forEach((id) => { const el = cockpitEl(id); if (el)
        el.classList.remove("cockpit-telao-slide-ativo"); });
