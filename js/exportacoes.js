...
const AJUDAS_UI = {
  ...
  webhook: { titulo:"Webhook e segurança", html:`<p>O webhook é a credencial usada pelo navegador para falar diretamente com seu Bitrix24.</p><ul><li>Ele <strong>nunca vem embutido</strong> no arquivo HTML.</li><li>Por padrão, fica somente na aba atual.</li><li>Ao clicar em <strong>Salvar webhook</strong>, a URL completa é armazenada no <code>localStorage</code> deste navegador e carregada automaticamente na próxima abertura.</li><li>Use esse recurso somente em computador pessoal ou confiável.</li><li>Em máquina compartilhada, use <strong>Esquecer webhook</strong> ao terminar.</li><li>As permissões do webhook determinam o que pode ser lido ou alterado.</li></ul><div class="help-example">Salvar webhook = persistir somente neste navegador. O valor não é escrito dentro do arquivo HTML.</div>` },
